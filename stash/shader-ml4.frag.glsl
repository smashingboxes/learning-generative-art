...
vec4 getSpace() {
  ...
  vec3 ro= roty(vec3(learning9*muscle1), learning1 + learning2 - muscle4);
  vec3 uu= normalize(cross(ro, vec3(muscle5, .0, 0.0)));
  vec3 vv= normalize(cross(uu, ro));
  vec3 rd= normalize(p.x*uu + p.y*vv - ro*0.1 );
  vec3 rm = log(raymarch(ro,rd))*learning5;
  vec3 preout = vec3( cr*rm.r, cg*rm.g, cg*rm.b ) / (muscle6/2.) + time;
